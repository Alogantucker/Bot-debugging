  ...
  client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (interaction.options.getAttachment('attachment')) {
        ...
        console.log(`the attachement __________________ ${attachment1}`);
      }
      ...
      switch (modalID) {
        ...
        case 'enterFileLinkModal':
          ...
          let links = (linkSlot1, linkSlot2, linkSlot3) => {
            ...
            if (linkSlot2) {
              newLinks = `${newLinks}2️⃣ - ${linkSlot2}\n`;
            }
            if (linkSlot3) {
              newLinks = `${newLinks}3️⃣ - ${linkSlot3}\n`;
            }
            return newLinks;
          }
        ...
        case 'newCustomModal':
          ...
          completeEmbed = await newCustomEmbed(client, interaction, targetChannel, attachment, role1, role2, role3, leakName, dateOfLeak, altLeakNames, price, notes);
          // send buttons to confirm send to desrired channels// if yes send Announcement
          console.log(`the embed - ------ ---- --- ---- -- ${completeEmbed}`);
          collectEmbed.set('embed', completeEmbed)
